window.onload = () => {
    axios.get("/js/member_center.json", {}).then((response) => {
    ...
    $("#memberInfo").on("click", "#btn_modify_member", (e) => {
        ...
        member["memLastName"] = document.getElementById("mem_lastname").value;
        member["memFirstName"] = document.getElementById("mem_firstname").value;
        ...
        axios
            .put(
                "http://localhost:8080/Proj_Yokult/api/0.01/member/modify",
                member
            )
            .then((response) => {
                let msg = response.data["msg"];
...
<label for="mem_id" class="form-label">帳號</label>
    ...
    <div class="input-group mb-3">
        <input type="text" id="mem_lastname" class="form-control" placeholder="請輸入姓氏" value="${
            member["memLastName"] ?? ""
        }"/>
        <input type="text" id="mem_firstname" class="form-control" placeholder="請輸入名字" value="${
            member["memFirstName"] ?? ""
        }"/>
